...
class NormalizedGenerationRequest:
    ...
    gemini_contents_raw: Optional[List[Any]] = None  # for multi-turn i2v
...
async def _extract_prompt_and_images_from_gemini_contents(
    ...
    # 扫描所有消息，收集 prompt 文本和图片（支持多轮 i2v：model 回复中的图+user 指令图）
    # 最后一个 user 消息决定 prompt；所有图片（含历史）合并收集
    prompt_parts: List[str] = []
    images: List[bytes] = []
    last_user_idx = -1
    ...
    # prompt 只取最后一个 user 消息的内容（过滤掉 model 回复中的文本）
    if last_user_idx >= 0:
        target_content = contents[last_user_idx]
        prompt_parts.clear()
...
async def _normalize_gemini_request(
    ...
    # 构建 NormalizedGenerationRequest，保留完整的 contents 用于多轮 i2v
    normalized = NormalizedGenerationRequest(
        model=resolved_model,
        prompt=prompt,
        images=images,
        contents=request.contents,          # 原始 GeminiContent 列表（支持多轮）
        system_instruction=request.systemInstruction,
        generation_config=request.generationConfig,
        gemini_contents_raw=None,
    )
    return normalized
...
async def _iterate_openai_stream(
    ...
    async for chunk in handler.handle_generation(
        ...
        stream=True,
        base_url_override=base_url_override,
        generation_config=normalized.generation_config,
