...
class GenerationHandler:
    ...
    async def handle_generation(
        ...
        generation_config: Optional[Any] = None,   # Gemini generationConfig
        contents: Optional[List[Any]] = None,      # Gemini contents (多轮 i2v)
        system_instruction: Optional[Any] = None,  # Gemini systemInstruction
        messages: Optional[List[Any]] = None,      # OpenAI messages
    ) -> AsyncGenerator:
        """统一生成入口
        ...
        Args:
            ...
            images: 图片列表 (bytes格式)
            stream: 是否流式输出
            generation_config: Gemini generationConfig (imageConfig / responseModalities)
            contents: Gemini contents 列表（支持多轮图生图：model回复+user指令+参考图）
            system_instruction: Gemini systemInstruction
            messages: OpenAI messages（备用）
        """
    ...
    async def _handle_image_generation(
        ...
        try:
            ...
            # 从 generationConfig.imageConfig 提取用户指定的配置覆盖
            effective_aspect_ratio = model_config["aspect_ratio"]
            effective_upsample = model_config.get("upsample")
            if generation_config and hasattr(generation_config, "imageConfig") and generation_config.imageConfig:
                ic = generation_config.imageConfig
                if ic.aspectRatio:
                    ratio_map = {
                    ...
                    mapped = ratio_map.get(ic.aspectRatio, ic.aspectRatio)
                    effective_aspect_ratio = mapped
                    debug_logger.log_info(f"[IMAGE] generationConfig.imageConfig 覆盖 aspect_ratio: {mapped}")
                if ic.imageSize:
                    size_map = {
                        "2K": "UPSAMPLE_IMAGE_RESOLUTION_2K",
                        "4K": "UPSAMPLE_IMAGE_RESOLUTION_4K",
                    }
                    if ic.imageSize in size_map:
                        effective_upsample = size_map[ic.imageSize]
                        debug_logger.log_info(f"[IMAGE] generationConfig.imageConfig 设置 upsample: {effective_upsample}")
            ...
            # 检查是否需要 upsample（优先使用 generationConfig.imageConfig 的 size 设置）
            upsample_resolution = effective_upsample  # 已由 imageConfig 覆盖（如有）
